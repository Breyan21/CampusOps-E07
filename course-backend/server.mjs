...
import { FIXTURE_ACCESS_TOKEN, FIXTURE_REFRESH_TOKEN, FIXTURE_REFRESH_TOKEN_NEXT } from './fixtures.mjs';
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === 'POST' && url.pathname === '/v1/session/refresh') {
    ...
    if (!input || input.refreshToken !== FIXTURE_REFRESH_TOKEN || scenario === 'invalid_refresh') {
      return send(response, 401, { code: 'invalid_grant' });
    }
    return send(response, 200, { accessToken: FIXTURE_ACCESS_TOKEN, refreshToken: FIXTURE_REFRESH_TOKEN_NEXT, expiresIn: 60 });
  }
